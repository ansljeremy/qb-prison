...
local function CreateCellsBlip()
	...
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentSubstringPlayerName("Vérification du temps de prison")
	...
	if PharmacyBlip then
		RemoveBlip(PharmacyBlip)
	end
	PharmacyBlip = AddBlipForCoord(Config.Locations["pharmacy"].coords.x, Config.Locations["pharmacy"].coords.y, Config.Locations["pharmacy"].coords.z)

	SetBlipSprite (PharmacyBlip, 61)
	SetBlipDisplay(PharmacyBlip, 4)
	SetBlipScale  (PharmacyBlip, 0.8)
	SetBlipAsShortRange(PharmacyBlip, true)
	SetBlipColour(PharmacyBlip, 0)
	BeginTextCommandSetBlipName("STRING")
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	canteen_ped = CreatePed(0, pedModel ,1786.19, 2557.77, 43.62, 186.04, false, true)
	...
	freedom_ped = CreatePed(0, pedModel , 1840.43, 2577.72, 45.01, 355.13, false, true)
		...
		options = {
			{
				...
				label = 'Vérification du temps de prison',
		...
		options = {
			{
				...
				icon = 'fas fa-clipboard',
				label = 'Obtenir de la nourriture',
				canInteract = function()
					return inJail
				end
			}
...
AddEventHandler('onResourceStart', function(resource)
	...
	canteen_ped = CreatePed(0, pedModel, 1780.6, 2560.78, 44.67, 176.64, false, true)
	FreezeEntityPosition(canteen_ped, true)
	SetEntityInvincible(canteen_ped, true)
	SetBlockingOfNonTemporaryEvents(canteen_ped, true)
	TaskStartScenarioInPlace(canteen_ped, 'WORLD_HUMAN_BUM_STANDING', 0, true)
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	TriggerEvent("chatMessage", "SYSTEM", "warning", "Votre bien a été saisi, vous récupérerez tout lorsque votre temps sera écoulé..")
	DoScreenFadeOut(500)
...
RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		...
		TriggerEvent("chatMessage", "SYSTEM", "warning", "vous avez récupéré votre propriété..")
		inJail = false
		RemoveBlip(currentBlip)
		RemoveBlip(CellsBlip)
		CellsBlip = nil
...
RegisterNetEvent('prison:client:canteen',function()
	local ShopItems = {}
	ShopItems.label = "Prison Cantine"
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				exports['qb-core']:DrawText('[E] Vérification du temps de prison', 'left')
			else
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				exports['qb-core']:DrawText('[E] Prendre son plateau', 'left')
			else
				exports['qb-core']:HideText()
			end
		end)
